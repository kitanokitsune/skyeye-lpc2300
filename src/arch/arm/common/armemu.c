...
#define ARNG(adr1, adr2) ((pc >= adr1) && (pc <= adr2+1))
#define CPSRMODE(n) (  (n)==16 ? "usr" : \
                       (n)==17 ? "fiq" : \
                       (n)==18 ? "irq" : \
                       (n)==19 ? "svc" : \
                       (n)==23 ? "abt" : \
                       (n)==31 ? "sys" : "und"   )
#define DBGTRACEPRT(s) fprintf(stderr,"----> PC=%08x\t[%s]\t#%ld\n",pc,s,state->NumInstrs)

        if ( ARNG(0x0, 0x1c) ) {
            DBGTRACEPRT("vector");
        } else if ( ARNG(0x914, 0x994) ) {
            DBGTRACEPRT("main");
        } else if ( ARNG(0x5f8, 0x728) ) {
            DBGTRACEPRT("sio_irq");
        }

        if ( 0
//           || ARNG(0x914,0x994) /* keil uart sample <main> */
//           || ARNG(0x058,0x150) /* keil uart sample <Reset_Handler> */
//           || ARNG(0xa20,0xa38) /* keil uart sample <getchar?> */
//           || ARNG(0xaa4,0xaa8) /* keil uart sample <getchar?> */
//           || ARNG(0xa28,0xa38) /* keil uart sample <getc> */
//           || ARNG(0xa7c,0xaa8) /* keil uart sample <__init> */
//           || ARNG(0x1b4,0x1d8) /* keil uart sample <fgetc> */
//           || ARNG(0x8b4,0x8fc) /* keil uart sample <com_getchar> */
//           || ARNG(0x5f8,0x728) /* keil uart sample <sio_irq> */
//           || ARNG(0x788,0x830) /* keil uart sample <com_initialize> */
            ) {
            fprintf(stderr, "#%ld\n", state->NumInstrs);
            fprintf(stderr, "PC: %08x, INSTR: %08x\n", pc & ~1, instr);
            fprintf(stderr," ------------------------------------------------------\n");
            ...
            fprintf(stderr,"|  r8=%08x, r9=%08x, SL=%08x, FP=%08x  |\n",
                    state->Reg[8],state->Reg[9],state->Reg[10],state->Reg[11]);
            fprintf(stderr,"|  IP=%08x, SP=%08x, LR=%08x, PC=%08x  |\n",
                    state->Reg[12],state->Reg[13],state->Reg[14],state->Reg[15]);
            fprintf(stderr,"|  CPSR=%08x  Flag(NZCV:IF)=%1d%1d%1d%1d:%1d%1d   Mode=%s     |\n",
                    state->Cpsr,NFLAG,ZFLAG,CFLAG,VFLAG,IFLAG,FFLAG,CPSRMODE(state->Cpsr&0x1f));
            fprintf(stderr,"|  SPSR | %08x %08x %08x %08x          |\n",
                    state->Spsr[0],state->Spsr[1],state->Spsr[2],state->Spsr[3]);
            fprintf(stderr,"|  [%01d]  | %08x %08x %08x                   |\n",
                    state->Bank,state->Spsr[4],state->Spsr[5],state->Spsr[6]);
            fprintf(stderr,"|  NirqSig=%08x  NfiqSig=%08x                  |\n",
                    state->NirqSig,state->NfiqSig);
            fprintf(stderr," ------------------------------------------------------\n");
            fflush(stderr);
        }
//        if (pc == 0x27c) exit(0);
        if ((state->Reg[13] > 0x3f000000) && (state->Reg[13] < 0x40000000)) {
